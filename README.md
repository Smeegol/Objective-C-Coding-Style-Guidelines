...
* 小括号与被它包含的内容之间不留空；
* 控制语句的条件小括号与外界之间留 1 个空格，如 `if`、`for`、`while`、`switch`；
...
* 集合类型尽量使用范型，集合遍历尽量使用 Block 式枚举，如 `enumerateWith...`，而不是 `for`、`while` 循环；
* 不必在 `init` 方法中把变量初始化为 0 或者 nil，多余； 
